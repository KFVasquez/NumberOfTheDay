...
function Menu() {
  ...
  const toggleDarkMode = () => {
    ...
    ? localStorage.setItem("light/dark", JSON.stringify("darkMode"))
    : localStorage.setItem("light/dark", JSON.stringify("lightMode"))
    
    setContextTheme(prevState => prevState === "lightMode" ? "darkMode" : "lightMode")
  }

  return (
    <div id='settingsDiv'>
      <div id="settingsSvg" onClick={toggleSettings}>
      ...
      <div id='menuSvg' onClick={toggleMenu}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 5.25h16.5m-16.5 4.5h16.5m-16.5 4.5h16.5m-16.5 4.5h16.5" /></svg>
      </div>
      <div id="settingsModesDiv" className={settingsOpen ? "visible" : ""}>
        <h4 id="light/dark" onClick={toggleDarkMode}>{contextTheme === "lightMode" ? "Modo nocturno" : "Modo día"}</h4>
        <h4><Link to="/rules">Reglas</Link></h4>
      </div>
      <div id='gameModesDiv' className={menuOpen ? "visible" : ""}>
        <h4><Link to="/">Número del día</Link></h4>
        <h4><Link to="/numberMinute">Número del minuto</Link></h4>
        <h4><Link to="/freeMode">Modo libre</Link></h4>
      </div>
    </div>
  )
}
